...
const MetricCard = ({ title, value, unit, icon: Icon, colorClass, aqiStatus = false }) => {
  // Ensure value is always a number
  const numericValue = typeof value === "number" && !isNaN(value) ? value : 0;

  // Calculate AQI details only if requested
  const { colorClass: aqiColorClass, status } = aqiStatus ? getAqiDetails(numericValue) : { colorClass: "", status: "" };

  return (
    <div className={`metric-card ${colorClass} text-white`}>
      <div className="card-top">
        <Icon size={24} style={{ opacity: 0.8 }} />
        <span className="card-value">{numericValue.toFixed(aqiStatus ? 0 : 1)}</span>
      </div>
      <div>
